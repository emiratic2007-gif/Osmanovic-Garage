  ...
  if (!post) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white p-4">
        ...
        <Button onClick={() => navigate("/blog")} className="bg-[#FF4500] rounded-none">
          Nazad na savjete
        </Button>
      </div>
    );
      ...
      <main className="pt-20">
        ...
        <div className="bg-gray-50 py-4 border-b border-gray-200">
          <div className="container mx-auto px-4 max-w-4xl flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-gray-400">
            <Link to="/" className="hover:text-[#FF4500]">Početna</Link>
            <ChevronRight className="w-3 h-3" />
            <Link to="/blog" className="hover:text-[#FF4500]">Savjeti</Link>
            <ChevronRight className="w-3 h-3" />
        ...
        <article className="pb-20">
          <div className="container mx-auto px-4 max-w-4xl pt-16">
            ...
            >
              <div className="flex items-center gap-4 mb-6 text-xs font-bold uppercase tracking-[0.2em] text-[#FF4500]">
                <span className="bg-[#FF4500]/10 px-3 py-1">{post.category}</span>
              ...
              <div className="flex items-center justify-between py-8 border-y border-gray-100 mb-12">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-[#111111] flex items-center justify-center text-white font-black text-xl">
                    OG
                  </div>
                  <div>
          ...
          <div className="container mx-auto px-4 max-w-3xl">
            <div className="prose prose-lg max-w-none prose-headings:font-black prose-headings:uppercase prose-headings:tracking-tighter prose-p:text-gray-600 prose-p:leading-relaxed prose-strong:text-[#111111] prose-a:text-[#FF4500] prose-li:text-gray-600">
              <ReactMarkdown>{post.content}</ReactMarkdown>
            </div>

            <div className="mt-20 pt-10 border-t border-gray-100">
              <Link to="/blog">
                <Button variant="outline" className="rounded-none border-2 border-[#111111] font-black uppercase tracking-widest hover:bg-[#111111] hover:text-white transition-all">
        ...
        <div className="container mx-auto px-4 max-w-7xl">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {BLOG_POSTS.filter(p => p.id !== id).slice(0, 3).map((relatedPost) => (
              <Link key={relatedPost.id} to={`/blog/${relatedPost.id}`} className="group bg-white p-6 shadow-sm hover:shadow-xl transition-all border-b-4 border-transparent hover:border-[#FF4500]">
                <div className="text-[10px] font-bold uppercase tracking-widest text-[#FF4500] mb-2">{relatedPost.category}</div>
                <h3 className="text-xl font-black uppercase tracking-tighter leading-tight mb-4 group-hover:text-[#FF4500] transition-colors line-clamp-2">
                  {relatedPost.title}
                </h3>
